...
f = np.arange(0, df*2049, df) # frequency half space used by YMAEDA_TOOLS
F = np.arange(0, df*4096, df) # frequency full space used by YMAEDA_TOOLS
...
def winv_lstsq(main_dir, w=0, DATLEN=2049, dt=0.1, N_DATA=6, N_MODEL=2):
    ...
    w = water level regularization parameter
    """
    M = np.zeros([DATLEN, N_MODEL]) #Y MAEDA_TOOLS calculated m
    for i in range(DATLEN):
